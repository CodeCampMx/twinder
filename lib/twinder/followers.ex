defmodule Twinder.Followers do
  ...
  def common_followers(username1, username2) do
    ...
    bff1 = for friend_id <- friends_for_u1, follower <- followers_of_u1, friend_id == follower.id, do: follower
    bff2 = for friend_id <- friends_for_u2, follower <- followers_of_u2, friend_id == follower.id, do: follower

    common_followers = for follower_u1 <- followers_of_u1, follower_u2 <- followers_of_u2, follower_u1.id == follower_u2, do: follower_u1
  end

  def common_bff(username1, username2) do
    #matches = for bff_of_u1 <- bff1, bff_of_u2 <- bff2, bff_of_u1.id == bff_of_u2, do: bff_of_u1
  end
